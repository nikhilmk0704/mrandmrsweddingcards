...
class Locatecustomerupdatemodel extends CI_Model
{
    function updateLocatecustomer($driver_token)
    {
        ...
        if ($userid != 'a') {
            ...
            // rejected request
            $sql = "SELECT `id_trip_request` FROM `trip_request` WHERE 1 AND `drivers_id`='" . $userid . "' AND `status`='1'";
            ...
            if ($trip->num_rows() > 0) {
                ...
                $where = array('id_trip_request' => $rowId->id_trip_request, 'status' => 1, 'drivers_id' => $userid);
                ...
                if ($tripDetail->num_rows() > 0) {
                    ...
                    $cancelledSql = "SELECT `id_trip_request` FROM `cancelled_trip_request` WHERE 1 AND `drivers_id`='" . $userid . "' AND `status`='1'";
